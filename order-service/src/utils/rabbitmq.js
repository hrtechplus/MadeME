...
class RabbitMQClient {
  constructor() {
    ...
    this.channel = null;
    this.url =
      process.env.RABBITMQ_URL || "amqp://mademe:mademe_password@rabbitmq:5672";
  ...
  async reconnect() {
    if (this.retryAttempts >= this.maxRetryAttempts) {
      console.error(
        `Maximum retry attempts (${this.maxRetryAttempts}) reached. Giving up on RabbitMQ connection.`
      );
      return;
    }
    ...
    this.retryAttempts++;
    console.log(
      `Attempting to reconnect to RabbitMQ (attempt ${this.retryAttempts}/${this.maxRetryAttempts}) in ${this.retryInterval}ms...`
    );
