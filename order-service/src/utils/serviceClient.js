...
class ServiceClient {
  constructor() {
    this.paymentServiceUrl = process.env.PAYMENT_SERVICE_URL || 'http://payment-service:5003';
    this.restaurantServiceUrl = process.env.RESTAURANT_SERVICE_URL || 'http://restaurant-service:5005';
    this.userServiceUrl = process.env.USER_SERVICE_URL || 'http://user-service:5004';
    this.cartServiceUrl = process.env.CART_SERVICE_URL || 'http://cart-service:5002';
  ...
  async createPaymentIntent(paymentData) {
    try {
      const response = await this.paymentService.post('/api/payments/initiate', paymentData);
  ...
  async getPaymentStatus(orderId) {
    try {
      const response = await this.paymentService.get(`/api/payments/order/${orderId}`);
      return response.data;
    } catch (error) {
  ...
  async getRestaurantById(restaurantId) {
    try {
      const response = await this.restaurantService.get(`/api/restaurants/${restaurantId}`);
      return response.data;
    } catch (error) {
      console.error(`Failed to get restaurant with ID ${restaurantId}:`, error.message);
      throw error;
    }
  }
  ...
  async getMenuItems(restaurantId) {
    try {
      const response = await this.restaurantService.get(`/api/restaurants/${restaurantId}/menu`);
      return response.data;
    } catch (error) {
      console.error(`Failed to get menu items for restaurant ${restaurantId}:`, error.message);
      throw error;
    }
  }
  ...
  async validateToken(token) {
    try {
      const response = await this.userService.post('/api/users/validate-token', { token });
      return response.data;
    } catch (error) {
